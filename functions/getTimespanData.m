function [hmT] = getTimespanData(mT, startSec, endSec, maxLatency)
    % 22: active lever 22
    ...
    copyVars = {'TagNumber', 'ID', 'Run', 'Session', 'sessionType', 'slideSession','Sex','TimeOfBehavior', 'LHbTarget', 'LHbAAV', 'Chamber'}; % SSnote: removed Acquire. will break things when I try to run it on most data. 
    hmT = mT(:, copyVars);
    ...
    for hv = 1:length(hourVars)
        ...
        for fl = 1:height(mT)
            EC = mT.eventCode{fl};
            ET = mT.eventTime{fl};
            EC = EC(ET >= startSec & ET <= endSec);
            ET = ET(ET >= startSec & ET <= endSec); 
